...
import pytest
from water_flow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, kpa_to_psi, pressure_loss_from_fittings,reynolds_number,pressure_loss_from_pipe_reduction
...
def test_reynolds_number():
    ...
    assert reynolds_number(0.048692, 1.75) == approx(84922,abs=11)
...
def test_kpa_to_psi():
    # Test cases for kpa_to_psi function
    assert kpa_to_psi(0.0) == approx(0.0, abs=0.001)
    assert kpa_to_psi(100.0) == approx(14.50377377, abs=0.001)
    assert kpa_to_psi(200.0) == approx(29.00754754, abs=0.001)
